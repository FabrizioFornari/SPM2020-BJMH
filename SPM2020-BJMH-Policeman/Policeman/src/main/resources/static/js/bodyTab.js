...
layui.define(["element","jquery"],function(exports){
	...
	Tab.prototype.render = function() {
		//显示左侧菜单
		var _this = this;
		$(".navBar ul").html('<li class="layui-nav-item layui-this"><a data-url="/main/main"><i class="layui-icon" data-icon=""></i><cite>Management</cite></a></li>').append(_this.navBar(dataStr)).height($(window).height()-210);
	...
	$("body").on("click",".top_tab li",function(){
		...
		if($(this).index() == 0){
			...
			if(window.sessionStorage.getItem("curmenu") == "undefined"){
				//如果Delete的不是当前选中的tab,则将curmenu设置成当前选中的tab
	...
	$("body").on("click",".top_tab li i.layui-tab-close",function(){
		//Deletetab后重置session中的menu和curmenu
		...
		if(menu != null) {
            ...
            if (JSON.stringify(curmenu) != JSON.stringify(menu[liIndex - 1])) {  //如果Delete的不是当前选中的tab
                // window.sessionStorage.setItem("curmenu",JSON.stringify(curmenu));
	...
	$(".closePageOther").on("click",function(){
		if($("#top_tabs li").length>2 && $("#top_tabs li.layui-this cite").text()!="Backend"){
			var menu = JSON.parse(window.sessionStorage.getItem("menu"));
			$("#top_tabs li").each(function(){
				if($(this).attr("lay-id") != '' && !$(this).hasClass("layui-this")){
					element.tabDelete("bodyTab",$(this).attr("lay-id")).init();
					//此处将当前窗口重新获取放入session，避免一个个Delete来回循环造成的不必要工作量
		...
		}else if($("#top_tabs li.layui-this cite").text()=="Management" && $("#top_tabs li").length>1){
			$("#top_tabs li").each(function(){
				if($(this).attr("lay-id") != '' && !$(this).hasClass("layui-this")){
					element.tabDelete("bodyTab",$(this).attr("lay-id")).init();
